...
actions in a row failed. The behavior kept asking for something the
game refused: a target that is not there, a command the character
cannot do, a container that does not open. The reason after the count
is what the last one answered, and the line under it is the game's own
words when the action read one, so the stop usually names its own
cause. The line after that says which behaviors above it declined the
