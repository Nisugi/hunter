...
module EO::Engine
  ...
  class Behavior
    ...
    # :failed, a stance no-op that says :success, a skipped line and a
    # nil are all ticks the game never heard, and none is a fire. More
    # than +fires+ of them inside any +seconds+ window trips the engine's
    # watchdog, the way repeated failures do. Sixty in a minute is faster
    # than roundtime allows a real action, so it only catches a behavior
    # looping on successes without the game slowing it (a retarget loop,
    # a re-search, a stance flip). Nil opts out; the trip behaviors
    # (Wander, Rest) do, since a walk can step through rooms faster than
    # that legitimately.
    #
